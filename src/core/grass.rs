...
impl GrassSystem {
    ...
    fn generate_grass_instances(planet_radius: f32, density: f32) -> Vec<InstanceData> {
        ...
        for _ in 0..num_blades {
            // Generate random spherical coordinates using uniform distribution
            let u: f32 = rng.gen();
            let v: f32 = rng.gen();
            ...
            let scale = 0.8 + rng.gen::<f32>() * 0.4; // 0.8 to 1.2
            ...
            // Color variation - slight variations in green
            let base_green = 0.4;
            let color_variation = Vec3::new(
                0.0,
                base_green + rng.gen::<f32>() * 0.2, // 0.4 to 0.6
                0.0,
            );
